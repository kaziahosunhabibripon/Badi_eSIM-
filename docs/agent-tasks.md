...
Auth is an optional bonus in the docx (role-based access), so keep it small. Reuse the existing 3
agents and 5 customers `backend/seed.py` already creates (plan section 35 coverage) - do not replace
them with a smaller, different set of demo accounts; add passwords to the ones that already exist.
...
- B1.3 New Alembic revision (on top of the existing one - do not edit it): `users.password_hash`
  String(255), NULLABLE. Users created implicitly by an agent's ticket (find-or-create) have NULL and
  cannot log in; document this known limitation (real systems invite the customer by e-mail).
  `password_hash` never appears in a response schema or a log.
...
- B1.5 Endpoints (root paths): `POST /auth/login {email,password}` ->
  `{access_token, token_type:"bearer", user:{id,name,email,role}}` (`id` is the real integer id, not a
  string). `GET /auth/me` -> current user. Every login failure is 401 `INVALID_CREDENTIALS` (code, in the
  standard `{"error":{code,message,details}}` shape - see app/core/exceptions.py) with the same generic
  message (unknown e-mail, wrong password, NULL hash); for an unknown e-mail still run a dummy hash
  verification so response time does not reveal which e-mails exist. Optional: `POST /auth/register` for
  customers (role forced to CUSTOMER, password at least 8 characters, 409 if the e-mail exists).
- B1.6 `deps.py`: replace `get_current_user`'s X-User-Id header with `Authorization: Bearer <token>`
  (`HTTPBearer(auto_error=False)` so errors still go through the AppError handlers, same shape as every
  other error). Missing -> 401 `NOT_AUTHENTICATED`; bad signature or format -> 401 `INVALID_TOKEN`;
  expired -> 401 `TOKEN_EXPIRED`; each with a `WWW-Authenticate: Bearer` header. Remove the X-User-Id
  header entirely. Remove `GET /users/demo` and the `DEMO_MODE` setting (superseded by real login). Keep
  `GET /users?role=` (agents only) - the frontend's assignee dropdown still needs it.
- B1.7 WebSocket: browsers cannot set headers, and tokens in URLs end up in server access logs, so
  authenticate with a first message instead of the current `?user_id=` query parameter. After accept, the
  client must send `{"type":"auth","token":"<jwt>"}` within 5 seconds; validate the token, load the user,
  check ticket access, and only then register the connection with the manager; otherwise close with 1008.
  Remove the `?user_id=` parameter from `backend/app/api/routes/websocket.py`.
- B1.8 Seed: every seeded user in `backend/seed.py` gets a password hash; ONE documented demo-only
  password for all seeded accounts (do not give each user a different password - there is nothing to
  protect in a seeded demo database, and a single shared password keeps the README simple); print the
  e-mail/role table at the end of `seed.py` (already does - keep it).
- B1.9 Tests on the real PostgreSQL test DB (`badi_support_test`): login ok / wrong password / unknown
  e-mail (identical response); no token, malformed, tampered and expired token (build one with a negative
  expiry) give the right 401 codes; a token for a user that no longer exists -> 401; a customer token
  cannot do agent actions (403) nor read another customer's ticket (404); websocket: no auth message ->
  closed, customer token on someone else's ticket -> closed, valid token receives `message.created`. Move
  every existing test from the `as_user`/X-User-Id fixture to tokens through ONE new fixture; do not leave
  both mechanisms half-supported in the test suite.
- B1.10 README.md (repo root): rewrite the demo-identity section into a real auth section (flow, env
  vars, demo accounts and the shared password, token lifetime, no refresh tokens, secret handling,
  limitation about implicitly-created customers with no password) - remove the X-User-Id/GET
  /users/demo text since neither will exist any more. Update the trade-offs table: plan section 6/39
  ("no passwords, demo identity") was superseded by this optional bonus.

VERIFY: pytest green on badi_support_test; `alembic upgrade head` and `alembic check` clean on a fresh
database; curl login with a seeded account, then `GET /auth/me` and `GET /tickets` with the token; a
tampered token -> 401 `INVALID_TOKEN`; the app refuses to start without `JWT_SECRET`. Paste real output.
...
- B2.2 Structure per plan §21: `src/api`, `components`, `pages`, `hooks`, `auth`, `types`, `layouts`,
  `routes`, `utils`, `App.tsx`, `main.tsx`. Routes: `/login`, `/tickets`, `/tickets/new`, `/tickets/:id`
  (placeholders).
...
- B3.2 Login page: labelled e-mail and password, client validation (e-mail format, non-empty), button
  ...
  box lists the seeded e-mails and the shared demo password from the README, shown only when
...
- B7.1 `useTicketSocket(ticketId, onMessage)`: open `ws(s)://<API host>/tickets/{id}/ws` (derived from
  `VITE_API_BASE_URL`); on open immediately send `{"type":"auth","token":<jwt>}` (per B1.7 - the backend
  authenticates the socket from this first message, not a query parameter, since tokens in URLs end up in
  server access logs); handle `{"event":"message.created","ticket_id","message":{...}}`: ignore other
  tickets and unknown events, dedupe by message id (the sender also gets the HTTP response), append in
  order. Clean up on unmount and when `ticketId` changes.
- B7.2 Reconnect with exponential backoff (1s, 2s, 4s ... max 15s, reset on success); after a reconnect
  refetch the ticket so nothing sent meanwhile is missed. If the server closes with 1008 (auth/access), do
  not retry: log out on an expired token, otherwise show "You no longer have access."
